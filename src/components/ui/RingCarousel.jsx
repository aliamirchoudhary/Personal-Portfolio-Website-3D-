...
function RingCarousel({ cards = [], accentColor = THEME.primary, initialCenter }) {
  ...
  const transformFor = (offset) => {
    ...
    const map = compact ? {
      0: { x: 0, z: -30, ry: 0, scale: 0.75, op: 1 },
      1: { x: 85, z: 10, ry: 12, scale: 0.85, op: 1 },
      2: { x: 140, z: 25, ry: 20, scale: 0.6, op: 1 },
    } : {
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, color: THEME.textPrimary, userSelect: "none" }}>
      ...
      <div
        ...
        onWheel={onWheel}
        style={{
          width: compact ? "calc(100% - 32px)" : "100%",
          maxWidth: 900,
